...
func _on_sound_button_pressed() -> void:
	...
	audio_container._choised()


func _on_controls_button_pressed() -> void:
	_hide_containers()
	controls_container.show()
	controls_container._choised()
...
func _hide_containers():
	for child in get_children():
		if child is BoxContainer: child.hide()
...
func center():
	...
	var viewport_heigh = 270
	#работает только если все элементы равны по размеру
	var pivot_offset = index*(focused_object.size.y + button_container.get_theme_constant("separation")) + focused_object.size.y/2
	button_container.position.y = viewport_heigh/2.0 - pivot_offset
